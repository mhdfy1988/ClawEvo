...
export type NodeType = 'Document' | 'Repo' | 'Module' | 'File' | 'API' | 'Command' | 'Rule' | 'Constraint' | 'Process' | 'Step' | 'Risk' | 'Skill' | 'State' | 'Decision' | 'Outcome' | 'Evidence' | 'Goal' | 'Intent' | 'Tool' | 'Mode' | 'Topic' | 'Concept' | 'Attempt' | 'Episode' | 'FailureSignal' | 'ProcedureCandidate' | 'Pattern' | 'FailurePattern' | 'SuccessfulProcedure';
export type EdgeType = 'documents' | 'contains' | 'defines' | 'applies_when' | 'requires' | 'forbids' | 'permits' | 'overrides' | 'next_step' | 'uses_skill' | 'supported_by' | 'derived_from' | 'conflicts_with' | 'supersedes' | 'produces';
...
export type ProvenanceSourceStage = 'legacy_unknown' | 'transcript_message' | 'transcript_custom' | 'transcript_compaction' | 'hook_message_snapshot' | 'document_raw' | 'document_extract' | 'tool_output_raw' | 'tool_result_persist' | 'tool_output_summary' | 'runtime_bundle' | 'checkpoint' | 'delta' | 'skill_candidate';
...
export type RuntimeContextCategory = 'activeRules' | 'activeConstraints' | 'openRisks' | 'recentDecisions' | 'recentStateChanges' | 'relevantEvidence' | 'candidateSkills';
...
export type RelationRetrievalStrategy = 'batch_adjacency' | 'single_source_fallback' | 'single_node_adjacency' | 'no_relation_sources';
...
export type KnowledgePromotionClass = 'failure_experience' | 'local_procedure' | 'stable_skill' | 'hard_constraint_candidate';
