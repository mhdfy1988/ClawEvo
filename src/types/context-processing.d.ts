import type { NodePromptPreferredForm, NodeType, RuntimeContextBundle, RuntimeContextCategory, RuntimeContextFixedSlot, RuntimeContextSelectionSlot } from './core.js';
export type ContextInputRouteKind = 'conversation' | 'tool_result' | 'transcript' | 'document' | 'experience_trace' | 'system';
export type ContextProcessingFallbackKind = 'raw_record' | 'sentence_split' | 'coarse_node' | 'unresolved_concept' | 'supported_by_only';
export type SemanticExtractionNodeTarget = Extract<NodeType, 'Rule' | 'Constraint' | 'Process' | 'Step' | 'Risk' | 'Skill' | 'State' | 'Decision' | 'Outcome' | 'Goal' | 'Intent' | 'Tool' | 'Mode' | 'Topic' | 'Concept'>;
...
export type CanonicalConceptId = 'context_compression' | 'knowledge_graph' | 'provenance' | 'checkpoint' | 'runtime_bundle' | 'traceability' | 'artifact_sidecar';
...
export type UtteranceClauseBoundary = 'sentence' | 'punctuation' | 'connector' | 'fallback';
...
export type ContextNoiseDisposition = 'drop' | 'evidence_only' | 'hint_only' | 'materialize';
export type ContextNoiseSignal = 'empty_span' | 'acknowledgement' | 'duplicate_clause' | 'low_information' | 'weak_topic_only' | 'manual_override';
...
export type ContextProcessingNodeCandidateDisposition = 'materialize' | 'summary_only';
...
export type ManualCorrectionTargetKind = 'concept_alias' | 'promotion_decision' | 'noise_policy' | 'semantic_classification' | 'node_suppression' | 'label_override';
