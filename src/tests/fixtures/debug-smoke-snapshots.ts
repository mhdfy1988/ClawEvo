  ...
  explanations: [
    {
      ...
      summary:
        'Risk "tool_output:build failed and is blocked by a sqlite timeout during migration step 4." is active in session scope with 1 linked edges. Provenance: raw / tool_output_raw / compact-context. Governance: state=raw; validity=freshness=active, confidence=0.95; prompt=eligible, raw, high, reserved, needs-evidence, needs-compression; scope=current=session, recall=session_primary, write=session_open, promote=stay. Derived from 1 node(s). Raw source id: session-debug-snapshot:risk. Evidence anchor: record session-debug-snapshot:risk. Semantic spans: 1 clause(s), e.g. sentence-1-clause-1="build failed and is blocked by a sqlite timeout during migration step 4.". Experience: attempt=partial, episode=open. Failure signals: tool_output:build failed and is blocked by a sqlite timeout during migration step 4.. Procedure candidate: steps=workflow:Step 4: produce a long current process explanation with enough detail to overflow the tiny debug, prerequisites=rule:Always preserve provenance when selecting context.. Critical steps: workflow:Step 4: produce a long current process explanation with enough detail to overflow the tiny debug. Selection: included in openRisks. Reason: open risk (raw:tool_output_raw, readiness:raw/high/reserved). Scope: session scope remains the primary recall source. Query: "why is the build blocked". Category budget: 43.',
      trace: {
        source: {
