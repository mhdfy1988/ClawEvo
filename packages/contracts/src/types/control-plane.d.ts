...
import type { Freshness, GraphNode, NodeType, Scope, SkillCandidate } from './core.js';
import type { ExplainRequest, ExplainResult, GraphNodeFilter, IngestResult, RawContextInput } from './io.js';
import type { PromptAssemblyContract, RuntimeContextWindowContract, RuntimeMessageSummary, RuntimeWindowLatestPointers, RuntimeWindowSource, ToolCallResultPair } from './runtime-context.js';
import type { EvaluationReport, StageObservabilityReport, StageObservabilityTrendPoint } from './evaluation.js';
export type ControlPlaneServiceName = 'governance-service' | 'observability-service' | 'import-service' | 'extension-registry' | 'autonomy-service' | 'workspace-catalog-service' | 'platform-event-service' | 'control-plane-facade' | 'control-plane-server' | 'importer-registry' | 'control-plane-client';
export type ControlPlaneApiSurface = 'runtime_api' | 'debug_api' | 'control_plane_service' | 'control_plane_api';
export type ControlPlaneReadonlySource = 'live_runtime_snapshot' | 'persisted_runtime_snapshot' | 'transcript_session_file';
...
export declare const RUNTIME_API_BOUNDARY: readonly ControlPlaneBoundaryDescriptor[];
export declare const DEBUG_API_BOUNDARY: readonly ControlPlaneBoundaryDescriptor[];
export declare const CONTROL_PLANE_SERVICE_BOUNDARY: readonly ControlPlaneBoundaryDescriptor[];
export declare const CONTROL_PLANE_API_BOUNDARY: readonly ControlPlaneBoundaryDescriptor[];
export declare const CONTROL_PLANE_READONLY_SOURCES: readonly ControlPlaneReadonlySource[];
...
export interface ControlPlaneRuntimeReadModelContract<TMessage = unknown> {
    ...
    inspectObservabilityDashboard(params: {
        stage?: string;
        sessionIds?: string[];
        sessionId?: string;
        sessionFile?: string;
        ...
        historyLimit?: number;
        limit?: number;
    }, facade: ControlPlaneFacadeContract): Promise<ControlPlaneObservabilityDashboardPayload<TMessage>>;
...
export type ObservabilityDashboardMetricKey = 'runtime_live_window_ratio' | 'runtime_transcript_fallback_ratio' | 'runtime_average_compressed_count' | 'runtime_average_final_message_count' | 'recall_noise_rate' | 'promotion_quality' | 'knowledge_pollution_rate' | 'high_scope_reuse_benefit' | 'high_scope_reuse_intrusion' | 'multi_source_coverage';
...
export declare const DEFAULT_OBSERVABILITY_ALERT_THRESHOLDS: Readonly<ObservabilityAlertThresholds>;
...
export type PlatformExtensionCapability = 'import_job_runner' | 'governance_policy' | 'observability_metric' | 'workspace_view' | 'event_consumer' | 'sdk_client';
...
export type AutonomyRecommendationClass = 'threshold_tuning' | 'import_strategy' | 'recall_strategy' | 'promotion_strategy' | 'low_risk_automation';
...
export type PlatformEventType = 'extension.registered' | 'governance.proposal_submitted' | 'governance.proposal_reviewed' | 'governance.proposal_applied' | 'governance.proposal_rolled_back' | 'governance.bulk_rolled_back' | 'observability.snapshot_recorded' | 'import.job_created' | 'import.job_completed' | 'import.job_failed' | 'workspace.policy_saved' | 'autonomy.recommendations_generated';
...
export declare const DEFAULT_IMPORT_SCHEDULER_POLICY: Readonly<ImportSchedulerPolicy>;
...
export interface ImportServiceContract {
    ...
    runJob(input: {
        ...
        materialize?: (normalizedInput: RawContextInput, record: PendingImportJobRecord, engine: {
            ingest(input: RawContextInput): Promise<IngestResult>;
...
export interface ExtensionRegistryContract {
    ...
    registerExtension(input: Omit<PlatformExtensionManifest, 'status'> & {
        status?: PlatformExtensionStatus;
