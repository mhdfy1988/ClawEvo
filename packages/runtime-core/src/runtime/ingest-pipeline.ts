...
import type { IngestResult, RawContextInput, RawContextRecord, RawContextSourceType } from '@openclaw-compact-context/contracts';
...
export class IngestPipeline {
  ...
  private buildSemanticSpanNode(
    ...
    span: SemanticSpan,
    nodeType: NodeType,
    conceptMatch?: import('@openclaw-compact-context/contracts').ConceptMatch
  ): GraphNode {
    const sourceRef = this.buildSourceRef(record);
    const kind = this.resolveKind(nodeType);
    const provenance = this.buildSemanticProvenance(record, sourceRef, evidenceNodeId);
