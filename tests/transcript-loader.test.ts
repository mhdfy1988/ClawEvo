...
test('loadTranscriptContextInput maps custom_message and compaction entries to finer semantic hints', async () => {
  ...
  const transcript = [
    ...
    JSON.stringify({
      ...
      content: '',
      display: 'Provenance must be preserved.',
      details: {
        note: 'Compressed content cannot be treated as raw content.'
      }
    }),
    JSON.stringify({
      ...
      summary: 'Next step: connect tool_result_persist first, then refresh the checkpoint.',
