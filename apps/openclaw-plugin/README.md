...
node apps/openclaw-plugin/dist/bin/openclaw-context-cli.js summarize --mode auto --text "请在 LLM 不可用时回退到代码摘要"
node apps/openclaw-plugin/dist/bin/openclaw-context-cli.js summarize --mode code --text "今天先把首页做成控制塔视角，并保留任务总览。"
node apps/openclaw-plugin/dist/bin/openclaw-context-cli.js summarize --model codex-cli/gpt-5-codex --text "请只走本机 Codex CLI"
node apps/openclaw-plugin/dist/bin/openclaw-context-cli.js summarize --model codex-oauth/gpt-5.4 --text "请只走 ChatGPT OAuth"
node apps/openclaw-plugin/dist/bin/openclaw-context-cli.js summarize --model openai-responses/gpt-5.4 --text "请只走 OpenAI Responses API"
node apps/openclaw-plugin/dist/bin/openclaw-context-cli.js summarize --model qwen-compatible/<your-qwen-model-id> --text "请用千问兼容层压缩这段文本"
node apps/openclaw-plugin/dist/bin/openclaw-context-cli.js summarize --model custom-responses/<your-responses-model-id> --text "请用通用 Responses 兼容层压缩这段文本"
node apps/openclaw-plugin/dist/bin/openclaw-context-cli.js summarize --provider codex-oauth --text "只限制到 provider，不显式指定模型"
...
node apps/openclaw-plugin/dist/bin/openclaw-context-cli.js roundtrip --mode llm --model codex-oauth/gpt-5.4 --text "今天先把首页做成控制塔视角，并保留任务总览。"
...
node apps/openclaw-plugin/dist/bin/openclaw-context-cli.js explain --mode auto --text "今天先把首页做成控制塔视角，并保留任务总览。" --limit 2
node apps/openclaw-plugin/dist/bin/openclaw-context-cli.js explain --mode llm --model codex-cli/gpt-5-codex --text "今天先把首页做成控制塔视角，并保留任务总览。" --limit 2
...
- `--provider <provider>`
  - 可以省略
  - 只在你想把这次调用限制在某一个 provider 上、但又不想显式写具体模型时再用
- 当前 `mode` 的主心智已经收成：
  - `llm`
    - 默认模式
    - 直接按当前模型 / 默认模型 / `catalog.providerOrder` 走 LLM provider
  - `auto`
    - 也是先走 `llm`
    - 但 LLM provider 都不可用时，会自动回退到 `code`
  - `code`
    - 只在你明确想看规则链摘要，或者需要兜底调试时再用
- `codex / codex-cli / codex-oauth / openai-responses`
  - 现在仍然保留
  - 但主要作为兼容模式，不再是默认推荐心智
- 默认模板只保留一份顺序源：
...
- 省略 `--mode` 时，默认就按 `llm` 处理；`--mode llm` 和默认行为一致
- `--mode llm` 会直接读取顶层 `catalog`，并通过通用 transport 创建运行时 provider
- `--mode auto` 现在是 `llm-first + code fallback`，不再等同于“先走 Codex 专用顺序”
- 当前已经能通过 `catalog + llm` 直接跑起来的包括：
  - `openai-compatible-chat-completions`
  - `openai-chat-completions`
  - `openai-compatible-responses`
  - `openai-responses`
- `codex` 这一段仍然可用，但现在更推荐通过 `--model codex-cli/...` / `--model codex-oauth/...` 进入，而不是把 transport 继续塞进 `mode`
...
- 省略 `--mode` 时默认已经是 `llm`
- `--model <provider>/<model>` 已成为当前推荐的模型选择入口
