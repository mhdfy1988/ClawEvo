...
node apps/openclaw-plugin/dist/bin/openclaw-context-cli.js summarize --model bailian/qwen3.5-plus --text "请用百炼千问压缩这段文本"
node apps/openclaw-plugin/dist/bin/openclaw-context-cli.js summarize --model volcengine/<your-ark-endpoint-id> --text "请用豆包 Seed 2.0 Pro 压缩这段文本"
...
CLI 现在支持通过配置文件覆盖默认 provider 顺序、模型和各 provider 参数。
...
- `volcengine` 这类 Ark provider，实际调用时经常要填 endpoint id，而不是控制台里展示的模型名；模板里的 `<your-ark-endpoint-id>` 就是这个意思
