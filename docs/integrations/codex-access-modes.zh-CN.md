...
openclaw-context-cli summarize --config .\openclaw.llm.config.json --mode codex --text "请按配置顺序尝试 Codex"
openclaw-context-cli summarize --mode auto --text "今天先把首页做成控制塔视角，并保留任务总览。" --json
```
...
openclaw-context-cli roundtrip --mode auto --text "今天先把首页做成控制塔视角，并保留任务总览。" --json
```
...
openclaw-context-cli explain --mode auto --text "今天先把首页做成控制塔视角，并保留任务总览。" --limit 2 --json
```
