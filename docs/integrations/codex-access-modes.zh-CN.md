...
- `--mode auto`
  - 当前已经改成 `llm-first`
  - 只是在所有 LLM provider 都失败时回退到 `code`
- 省略 `--mode`
  - 当前默认按 `llm` 处理
  - 优先走当前模型 / 默认模型 / `catalog.providerOrder`
...
openclaw-context-cli summarize --mode auto --text "请在 LLM 不可用时回退到代码摘要"
openclaw-context-cli summarize --mode code --text "今天先把首页做成控制塔视角，并保留任务总览。"
openclaw-context-cli summarize --model codex-cli/gpt-5-codex --text "请只走本机 Codex CLI"
openclaw-context-cli summarize --model codex-oauth/gpt-5.4 --text "请只走 ChatGPT OAuth"
openclaw-context-cli summarize --model openai-responses/gpt-5.4 --text "请只走 OpenAI Responses API"
openclaw-context-cli summarize --model qwen-compatible/<your-qwen-model-id> --text "请用千问兼容层压缩这段文本"
openclaw-context-cli summarize --model custom-responses/<your-responses-model-id> --text "请用通用 Responses 兼容层压缩这段文本"
openclaw-context-cli summarize --provider codex-oauth --text "只限制到 provider，不显式指定模型"
...
openclaw-context-cli roundtrip --mode llm --model codex-oauth/gpt-5.4 --text "今天先把首页做成控制塔视角，并保留任务总览。"
...
openclaw-context-cli explain --mode auto --text "今天先把首页做成控制塔视角，并保留任务总览。" --limit 2
openclaw-context-cli explain --mode llm --model codex-cli/gpt-5-codex --text "今天先把首页做成控制塔视角，并保留任务总览。" --limit 2
...
- `--model <provider>/<model>` 是当前主推荐入口
- `--provider <provider>` 可以省略，只在你想限制到某一个 provider、但又不想显式写模型时再用
...
- 当前 `llm-toolkit` 已经把通用多厂商接入拆成两条 transport：
  ...
  - `openai-compatible-responses`
- 省略 `--mode` 时，默认就按 `llm` 处理
- `openclaw-context-cli --mode llm` 会优先使用 `catalog` 里的 provider 配置，而不是再走 Codex 专用顺序
- `openclaw-context-cli --mode auto` 现在是 `llm-first + code fallback`
- 这意味着：
  - CLI 的主体验已经切到 `llm-first`
  - `codex / codex-cli / codex-oauth / openai-responses` 这些 mode 仍然保留，但主要用于兼容旧命令
  - `--model <provider>/<model>` 已成为当前主推荐入口；`--provider` 只是可选高级筛选
  - 千问、豆包 / Ark、Ollama、LM Studio、Responses 兼容网关已经可以通过统一配置和统一 transport 接入
