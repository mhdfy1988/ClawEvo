...
## 1. 一句话目标
`导入平台负责编排 source-specific parser / normalizer / materializer 流程，并通过 runtime engine 入图；它不负�?provider-specific prompt 组装。`

## 2. 第一轮范�?这一轮固定了三类来源�?- `document`
- `repo_structure`
- `structured_input`
...
第一轮的 `ImportJob` 不再只有 `sourceKind` �?`status`，而是显式包含�?- `source`
...
## 8. 当前边界
这一轮已经做完的�?- import job model
- source-specific flow contract
- incremental / version / failure 字段
- gateway 生命周期
- 通过 runtime engine �?materialize 闭环

这一轮还没做的：
- 真正的文档解析器 / 仓库扫描�?- 批量 source catalog
- import history 持久�?- import dashboard
- import retry policy

## 9. 对阶�?6 的意�?`TODO 6` 第一轮完成后，control plane 已经不再只是“知道可以导入”，而是已经具备�?- job
- flow
- stage trace
- failure trace
- gateway lifecycle
