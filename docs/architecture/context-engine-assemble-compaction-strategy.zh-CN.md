...
`compact-context` 作为 OpenClaw 的 context engine 插件，应以 `assemble()` 作为自动上下文治理主入口：`ingest()` 负责建图谱，`assemble()` 负责组织上下文，并按“baseline + 1块 incremental + 最近2轮 rawTail”工作；其中 `incremental` 只覆盖 baseline 之后、rawTail 之前的那一段内容，超过 50%~60% 阈值时则在 `assemble()` 内直接做一次全量重压缩；`compact()` 只保留为手动 / 兼容入口。`
