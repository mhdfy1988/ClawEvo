...
1. 当前项目的 Codex transport 有默认 fallback 顺序，但允许被配置文件覆盖：
   ...
   - 如果存在 `compact-context.llm.config.json` 或显式 `--config`，则优先按配置文件里的顺序设置
   - 其中：
...
7. 当前项目要把“长期默认模型”和“临时当前模型”拆开：
   ...
   - 当前模型进状态文件 `compact-context.llm.state.json`
   不要把临时切换直接写回主配置。
...
10. 清空长期默认模型时，如果当前本来就没有配置文件，不要为了“清空”额外在 cwd 生成一个空的 `compact-context.llm.config.json`；默认状态应继续保持为真正的无配置状态。

## 适用任务

- 改 `compact-context.llm.config.json`
- 改模型切换 / 当前模型 / 默认模型
- 改 `providerOrder`
- 改 `--config` 查找链路
