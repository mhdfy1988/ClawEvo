...
这份文档把当前 `src/tests` 的测试归属固定下来，避免 package、app、smoke、evaluation 再次混回同一层。
...
- package tests
  - 不再依赖 root 固定 `dist`
  - 统一走 run-scoped 编译目录
  - 只验证各自 workspace 的共享行为
- app tests
  - 只验证薄壳入口、依赖、manifest 和壳层导出
  - 不重复覆盖 runtime/core 细节
- root smoke
  - 只保留跨 workspace 的边界验收
  - `required` 不包含 `debug-smoke`
...
后面如果要往 `src/tests` 的 root smoke / evaluation 再新增测试，应该先回答一句话：

`为什么这条测试不能下沉到 package unit 或 app integration？`
