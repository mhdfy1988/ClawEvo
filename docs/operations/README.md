# Operations 文档说明

这个目录放调试、排障、观察和运维侧文档，重点回答：
- gateway 调试怎么做
- 故障注入和 smoke 检查怎么做
- control-plane 发布和日常巡检怎么做
- 出问题时先看哪里

适合放在这里的文档：
- gateway debug usage
- debug playbook
- fault injection / smoke checklist
- control-plane release checklist
- control-plane production runbook

推荐先看：
1. [gateway-debug-usage.zh-CN.md](/d:/C_Project/openclaw_compact_context/docs/operations/gateway-debug-usage.zh-CN.md)
2. [debug-playbook.zh-CN.md](/d:/C_Project/openclaw_compact_context/docs/operations/debug-playbook.zh-CN.md)
3. [fault-injection-smoke-checklist.zh-CN.md](/d:/C_Project/openclaw_compact_context/docs/operations/fault-injection-smoke-checklist.zh-CN.md)
4. [control-plane-release-checklist.zh-CN.md](/d:/C_Project/openclaw_compact_context/docs/operations/control-plane-release-checklist.zh-CN.md)
5. [control-plane-production-runbook.zh-CN.md](/d:/C_Project/openclaw_compact_context/docs/operations/control-plane-production-runbook.zh-CN.md)
