...
这份文档用于收敛阶段 6 第一轮与第二轮已经落地的 dashboard �?observability contract�?
...
## 1. 一句话结论
`observability` 现在不再只是阶段报告 formatter，而是已经能输�?`dashboard contract + snapshot history + metric series + alerts`�?
## 2. 第一轮交�?
### 2.1 `ObservabilityDashboardContract`
第一�?dashboard contract �?5 部分组成�?1. `readonlySources`
2. `latestStageReport`
...
### 2.2 `buildDashboard(...)`
[observability-service.ts](/d:/C_Project/openclaw_compact_context/packages/compact-context-core/src/observability-service.ts) 已支持：
- `buildStageReport(...)`
...
### 5.2 未知状�?如果当前没有 stage reports �?runtime windows，对�?card 不会强行�?0，而是�?- `status = unknown`

## 6. 当前边界
已完成：
- dashboard contract
- 固定指标卡片
- 最小告警阈�?- runtime snapshot 一等数据源
- snapshot history
- metric history series
- gateway inspection

还没做完�?- 真正�?control plane query API
- 策略配置持久化管�?- dashboard 历史分页
- Web UI

## 7. 对阶�?6 的意�?这一轮完成后，阶�?6 �?observability 已经不只是：
- `report`
- `snapshot`
- `dashboard card`
- `alert`

还进一步具备了�?- `snapshot history`
- `metric series`
- `history inspection`

所以后面的 control-plane API �?Web UI 可以直接建立在这�?contract 之上�?
