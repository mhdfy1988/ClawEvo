...
相关代码�?- [contracts.ts](/d:/C_Project/openclaw_compact_context/packages/compact-context-core/src/contracts.ts)
- [governance-policy.ts](/d:/C_Project/openclaw_compact_context/packages/compact-context-core/src/governance-policy.ts)
- [governance-service.ts](/d:/C_Project/openclaw_compact_context/packages/compact-context-core/src/governance-service.ts)
- [observability-service.ts](/d:/C_Project/openclaw_compact_context/packages/compact-context-core/src/observability-service.ts)
- [import-service.ts](/d:/C_Project/openclaw_compact_context/packages/compact-context-core/src/import-service.ts)
- [control-plane-facade.ts](/d:/C_Project/openclaw_compact_context/packages/compact-context-core/src/control-plane-facade.ts)
- [control-plane-services.test.ts](/d:/C_Project/openclaw_compact_context/tests/control-plane-services.test.ts)

配套文档�?- [control-plane-api-matrix.zh-CN.md](/d:/C_Project/openclaw_compact_context/docs/control-plane/control-plane-api-matrix.zh-CN.md)
...
## 1. 一句话边界
`Runtime Plane 继续掌握在线上下文处理与知识写入权威；Control Plane 通过稳定 contract �?runtime / readonly source，并且不直接写底�?SQLite。`

## 2. API 分层
当前代码已经把四类入口分开�?1. `runtime API`
   - `bootstrap`
   - `ingest`
   - `ingestBatch`
   - `afterTurn`
...
这些边界已经通过 `ControlPlaneBoundaryDescriptor` 固定下来，所有项都带�?- `surface`
- `readonly`
- `authority`
- `directStoreAccess=false`

## 3. 只读数据�?当前 control plane 固定只认三类只读来源�?- `live_runtime_snapshot`
- `persisted_runtime_snapshot`
- `transcript_session_file`

它们分别代表�?1. `live_runtime_snapshot`
...
并通过 gateway 暴露�?- `compact-context.submit_correction_proposal`
...
2. `workspace`
   - `workspace_reviewer / global_reviewer` �?submit / review / apply / rollback
3. `global`
   - �?`global_reviewer` �?submit / review / apply / rollback
...
因此�?- `apply` 通过 engine 应用 corrections
- `rollback` 通过生成反向 corrections 再回�?
## 5. Observability Service
当前 observability service 负责�?1. `buildStageReport`
...
同时 gateway 暴露�?- `compact-context.inspect_observability_dashboard`
- `compact-context.capture_observability_snapshot`
- `compact-context.inspect_observability_history`

这意味着 observability 已经不再只是阶段报告 helper，而是有了�?- 当前�?dashboard
- snapshot 记录
...
4. `scheduleJob / runDueJobs`
   - 提供最小调度与批处理入�?5. `getJob / listJobs / getJobHistory`
   - 查询任务状态与 attempt history

同时 gateway 暴露�?- `compact-context.create_import_job`
...
三类能力收成一个稳�?service surface，用于承接：
- gateway handler
- 未来独立 control-plane API
- 未来 Web UI / console

这意味着上层已经不需要直接感知多�?service 的组合细节�?
## 8. 当前还没做完�?虽然 control plane 第一轮与第二轮都已落地，但还缺：
1. 更细的治理权限模�?2. 更成熟的审批策略
3. 独立进程�?control-plane API
4. 更完整的 import source-specific orchestration
5. dashboard 配置与告警策略持久化

## 9. 对阶�?6 的意�?这一轮的完成意味着�?- `TODO 3` 已经不只是设计想法，而是实际代码边界
- `TODO 4` 已有可用治理闭环，并�?runtime snapshot trace
- `TODO 5` 已能直接消费 runtime snapshot �?dashboard bundle �?history
- `TODO 6` 已可直接复用 import job contract，并具备第二轮治理能�?
